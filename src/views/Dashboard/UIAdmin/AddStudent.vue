...
const getAllStudent = async () => {
  const response = await fetchStudents(currentPage.value, pageSize.value);
  itemStudentTable.value = response.data.students.data;
  totalStudent.value = response.data.students.total;
};
...
const fetchStudents = async (page, perPage) => {
  return axios.get(`http://127.0.0.1:8000/api/get-all-students?page=${page}&per_page=${perPage}`);
};
...
const deleteStudent = async () => {
  if (studentIdToDelete) {
    try {
      const response = await axios.delete(`http://127.0.0.1:8000/api/admin/delete-student/${studentIdToDelete}`);
